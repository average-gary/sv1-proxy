...
use roles_logic_sv2::parsers::{CommonMessages, Mining, MiningDeviceMessages};
use std::sync::{
...
pub async fn handle_miner_sv2(
    ...
    loop {
        match node.receiver.recv().await {
            Ok(frame) => {
                let mut incoming: StdFrame = match frame.try_into() {
                    Ok(frame) => frame,
                    Err(e) => {
                        println!("Error converting frame: {:?}", e);
                        continue;
                    }
                };
                ...
                let message_type = match incoming.get_header() {
                    ...
                    None => {
                        println!("No header found in frame");
                        continue;
                    }
                };
                let payload = incoming.payload();
                let message: Result<MiningDeviceMessages<'_>, Sv2Error> = (message_type, payload).try_into();
                
                match message {
                    Ok(MiningDeviceMessages::Mining(Mining::OpenStandardMiningChannel(m))) => {
                    ...
                    Ok(MiningDeviceMessages::Mining(Mining::OpenExtendedMiningChannel(m))) => {
                    ...
                    Ok(MiningDeviceMessages::Mining(Mining::UpdateChannel(message))) => {
                    ...
                    Ok(MiningDeviceMessages::Mining(Mining::SubmitSharesStandard(message))) => {
                    ...
                    Ok(MiningDeviceMessages::Mining(Mining::SubmitSharesExtended(message))) => {
                        // TODO: convert from Sv2 to Sv1 share to then submit via the share_tx
                    ...
                    Ok(MiningDeviceMessages::Mining(Mining::SetCustomMiningJob(message))) => {
                        ...
                        println!("Received SetCustomMiningJob: channel_id={}, request_id={}, min_ntime={}, version={}, prev_hash={:?}, nbits={}, coinbase_tx_version={}, coinbase_prefix={:?}, coinbase_tx_input_n_sequence={}, coinbase_tx_value_remaining={}, coinbase_tx_outputs={:?}, coinbase_tx_locktime={}, merkle_path={:?}, extranonce_size={}, token={:?}",    
                    ...
                    Ok(MiningDeviceMessages::Common(CommonMessages::SetupConnection(message))) => {
                        println!("Received SetupConnection: {:?}", message);
                        let response = MiningDeviceMessages::Common(CommonMessages::SetupConnectionSuccess(
                            SetupConnectionSuccess {
                                used_version: 2,
                                flags: message.flags.into(),
                            }
                        ));
                        println!("Sending SetupConnectionSuccess: {:?}", response);
                        let response_frame: Sv2Frame<MiningDeviceMessages, Vec<u8>> = response.try_into().unwrap();
                        let either_frame: Frame<MiningDeviceMessages<'static>, Vec<u8>> = response_frame.into();
                        let _ = sender.send(either_frame);
                    }
                    Ok(MiningDeviceMessages::Common(CommonMessages::SetupConnectionSuccess(message))) => {
                        println!("Received SetupConnectionSuccess: {:?}", message);
                        println!("Why?");
                    }
                    Ok(MiningDeviceMessages::Common(CommonMessages::SetupConnectionError(message))) => {
                        println!("Received SetupConnectionError: {:?}", message);
                    }
                    Ok(MiningDeviceMessages::Common(CommonMessages::ChannelEndpointChanged(message))) => {
                        println!("Received ChannelEndpointChanged: {:?}", message);
                    }
